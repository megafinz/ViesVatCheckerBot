...
async function processVatValidationResult(
  ...
  if (now.getTime() > vatRequest.expirationDate.getTime()) {
    ...
    await deps.telegram.sendMessage(
      vatRequest.telegramChatId,
      `🔴 You VAT number '${vatNumber}' is no longer monitored because it's still invalid and it's been too long since you registered it. Make sure you entered the right VAT number or that the entity that this VAT number belongs to actually applied for registration in VIES.`
    );
    return true;
  }
...
async function demoteAndNotify(
  ...
  await deps.repository.demoteVatRequestToError(vatRequest, message);
  await deps.telegram.sendMessage(
    vatRequest.telegramChatId,
