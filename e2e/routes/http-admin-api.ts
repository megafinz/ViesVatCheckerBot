...
router.post('/resolveError', async (req, res) => {
  ...
  const result = await httpAdminApiHandlers.resolveError(console.log, errorId, silent);
  res.status(result.status).send(result.body);
});

router.post('/resolveAllErrors', async (req, res) => {
  const silent = req.query.silent || req.body?.silent;
  const result = await httpAdminApiHandlers.resolveAllErrors(console.log, silent);
